...
import { tk, MONO, masteryColor, masteryLevel } from "../../tokens";
import { useInstructorModule } from "../../store/InstructorStore";
import {
  Card, Btn, Chip, ScoreValue, BackCircle, inputStyle, bFontFor, hFontFor, Th,
} from "../../components/ModuleUI";
import { MasteryBar } from "../../components/SharedUI";
import { IconWarning, IconUsers } from "../../components/Icons";
import StudentInterventionModal from "../../components/StudentInterventionModal";
import {
  STUDENTS, STUDENT_INSTITUTIONAL_IDS, studentTopicMastery, latestAttempt, fmtWhen, INSTRUCTOR_COURSE_IDS,
} from "../../data/instructorModule";
import { COURSES } from "../../data/instructorModule";

// ─────────────────────────────────────────────────────────────────────────────
// Students — the instructor's roster per course: search + filters (cohort,
// mastery band, trend, open work), and a full student file (topic mastery,
// submission history with AI-vs-final scores, remedial received).
// Roster & cohort membership are registrar/admin-owned; this view is
// read-only by design — the instructor filters, never edits enrolment.
// ─────────────────────────────────────────────────────────────────────────────

export default function StudentsScreen({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  // demo roster: all students are enrolled in every institutional course;
  // the per-course unit history is what differentiates them
  const roster = STUDENTS;

  const unitsOf = (sid: string) => mod.units.filter((u) => u.studentId === sid && u.courseId === courseId);
      ...
      <div className="genai-pad" style={{ padding: "26px 32px", direction: isRtl ? "rtl" : "ltr" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, marginBottom: 20, flexDirection: isRtl ? "row-reverse" : "row" }}>
          <div style={{ display: "flex", gap: 14, alignItems: "flex-start", flexDirection: isRtl ? "row-reverse" : "row" }}>
            <BackCircle tokens={tokens} rtl={isRtl} onClick={() => setState({ ...state, studentId: undefined })} />
            <div style={{ textAlign: isRtl ? "right" : "left" }}>
              <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 4, flexWrap: "wrap", flexDirection: isRtl ? "row-reverse" : "row" }}>
                <h1 style={{ fontFamily: hFont, fontWeight: 700, fontSize: 22, color: tokens.textPrimary, letterSpacing: "-0.025em", margin: 0 }}>{s.name}</h1>
        ...
        <div className="genai-tiles-5" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 14, marginBottom: 18 }}>
          {[
            ...
            { l: lang === "ar" ? "معلّق الآن" : "OPEN NOW", v: `${units.filter((u) => u.status === "awaiting_review" || u.status === "resubmission_requested").length}`, c: tokens.textPrimary },
            ...
            {course.topics.map((t, i) => {
              ...
              return (
                <div key={t.id} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 0", borderTop: i > 0 ? `1px solid ${tokens.cardBorder}` : "none", flexDirection: isRtl ? "row-reverse" : "row" }}>
                  ...
                  <div style={{ width: 110, flexShrink: 0 }}><MasteryBar pct={pct} evidence={1} thin tokens={tokens} /></div>
                  <span style={{ fontFamily: MONO, fontSize: 11, fontWeight: 700, color: masteryColor(masteryLevel(pct, true), tokens), width: 34, textAlign: "right" }}>{pct}%</span>
                </div>
              );
            })}
          </Card>
  ...
  return (
    <div className="genai-pad" style={{ padding: "26px 32px", direction: isRtl ? "rtl" : "ltr" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, marginBottom: 18, flexWrap: "wrap", flexDirection: isRtl ? "row-reverse" : "row" }}>
        ...
        <select value={courseId} onChange={(e) => { setCourseId(e.target.value); setState({ ...state, courseId: e.target.value }); }} style={{ ...inputStyle(tokens, bFont), cursor: "pointer", width: 240 }} className="genai-input">
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16, flexDirection: isRtl ? "row-reverse" : "row" }}>
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={lang === "ar" ? "بحث بالاسم أو الرقم…" : "Search name or number…"} style={{ ...inputStyle(tokens, bFont), width: 220 }} className="genai-input" />
        <select value={fCohort} onChange={(e) => setFCohort(e.target.value)} style={{ ...inputStyle(tokens, bFont), cursor: "pointer", width: 150 }} className="genai-input">
        ...
        <select value={fBand} onChange={(e) => setFBand(e.target.value)} style={{ ...inputStyle(tokens, bFont), cursor: "pointer", width: 160 }} className="genai-input">
        ...
        <select value={fTrend} onChange={(e) => setFTrend(e.target.value)} style={{ ...inputStyle(tokens, bFont), cursor: "pointer", width: 140 }} className="genai-input">
        ...
        <select value={fWork} onChange={(e) => setFWork(e.target.value)} style={{ ...inputStyle(tokens, bFont), cursor: "pointer", width: 170 }} className="genai-input">
      ...
      <Card tokens={tokens} style={{ padding: "6px 20px" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {filtered.map((s, i) => {
              const open = unitsOf(s.id).filter((u) => u.status === "awaiting_review" || u.status === "resubmission_requested").length;
              return (
                <tr key={s.id} style={{ borderBottom: i < filtered.length - 1 ? `1px solid ${tokens.cardBorder}` : "none" }}>
                  ...
                  <td style={{ padding: "11px 10px", textAlign: "right" }}>
                    <span style={{ fontFamily: MONO, fontSize: 12, fontWeight: 700, color: open > 0 ? tokens.primary : tokens.textFaint }}>{open}</span>
                  </td>
                  <td style={{ padding: "11px 10px", textAlign: "right" }}>
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      <Btn tokens={tokens} lang={lang} variant="soft" style={{ padding: "6px 12px", fontSize: 11.5 }} onClick={() => setState({ ...state, studentId: s.id, courseId })}>
      ...
      <StudentInterventionModal open={interveneFor !== null} onClose={() => setInterveneFor(null)} studentId={interveneFor} courseId={courseId} tokens={tokens} lang={lang}
        onOpenFile={(id) => setState({ ...state, studentId: id, courseId })} />
    </div>
  );
}
