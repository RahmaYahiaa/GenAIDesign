...
import {
  Card, Btn, Chip, inputStyle, textareaStyle, bFontFor, hFontFor, toast, Skeleton,
} from "../../components/ModuleUI";
import {
  IconSparkle, IconDoc, IconPencil, IconImageAttach, IconClipboard, IconSend,
  IconDoubleCheck, IconInbox, IconRefresh,
} from "../../components/Icons";
import { COURSES, INSTRUCTOR_COURSE_IDS } from "../../data/instructorModule";

// ─────────────────────────────────────────────────────────────────────────────
// Content Studio — the instructor-side generation surface (the "AI tutor"
// counterpart for staff): pick ANY topic (course-mapped or free-form), pick a
// modality (text / worked examples / diagrams / video script / audio
// narration / quiz / slides / flashcards), generate an editable draft.
// Nothing is auto-published: drafts can be saved, or filed as pending course
// material for the approval gate.
// ─────────────────────────────────────────────────────────────────────────────

type ContentType = "text" | "examples" | "images" | "video" | "audio" | "quiz" | "slides" | "flashcards";

const TYPES: { id: ContentType; en: string; ar: string; Icon: typeof IconDoc }[] = [
...
function generateDraft(type: ContentType, topicLabel: string, courseId: string, length: string, tone: string, extra: string): string {
...
export default function ContentStudioScreen({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  const [courseId, setCourseId] = useState(state.courseId ?? "CS301");
  ...
  const generate = () => {
    ...
    window.setTimeout(() => {
      ...
      setDraftType(type);
      setBusy(false);
      toast(lang === "ar" ? "تم توليد المسودة — راجعها وعدّلها قبل أي نشر." : "Draft generated — review and edit before anything is published.");
    }, 900);
  };

  const Diagram = ({ n }: { n: number }) => (
    ...
    <div className="genai-pad" style={{ padding: "26px 32px", direction: isRtl ? "rtl" : "ltr" }}>
      <div style={{ marginBottom: 20, textAlign: isRtl ? "right" : "left" }}>
        ...
        <p style={{ fontSize: 13, color: tokens.textMuted, margin: 0, fontFamily: bFont }}>
          {lang === "ar"
            ? "ولّد محتوى تعليمياً بأي صيغة — على أي موضوع، سواء من خريطة المقرر أو موضوع حر تكتبه بنفسك."
            : "Generate teaching content in any modality — on any topic, course-mapped or free-form."}
        </p>
      </div>

      <div className="genai-grid-builder" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) minmax(0, 1.15fr)", gap: 24, alignItems: "start" }}>
        {/* controls */}
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <Card tokens={tokens} style={{ padding: "18px 20px" }}>
            ...
            {topicId === "__custom" && (
              <div style={{ marginTop: 12 }}>
                {mono(lang === "ar" ? "اكتب الموضوع الحر" : "FREE-FORM TOPIC")}
          ...
          <Card tokens={tokens} style={{ padding: "18px 20px" }}>
            ...
            <div style={{ marginTop: 14 }}>
              ...
              <textarea value={extra} onChange={(e) => setExtra(e.target.value)} rows={2} placeholder={lang === "ar" ? "مثال: ركّز على مقارنة التعقيدات." : "e.g. emphasise the complexity comparison."} style={textareaStyle(tokens, bFont)} className="genai-input" />
            </div>
          ...
          {busy ? (
            ...
            <Card tokens={tokens} style={{ padding: "18px 20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 10, flexDirection: isRtl ? "row-reverse" : "row" }}>
                <div style={{ fontFamily: hFont, fontWeight: 600, fontSize: 15, color: tokens.textPrimary, letterSpacing: "-0.02em" }}>
                  {TYPES.find((t) => t.id === draftType)?.Icon && <span style={{ display: "inline-flex", verticalAlign: "middle", marginInlineEnd: 8 }}><IconSparkle size={14} color={tokens.primary} /></span>}
                  {topicLabel}
                </div>
            ...
            <Card tokens={tokens} style={{ padding: "34px 24px", textAlign: "center" }}>
              ...
              <div style={{ fontFamily: hFont, fontWeight: 600, fontSize: 15, color: tokens.textPrimary, marginBottom: 4 }}>
                {lang === "ar" ? "اختر موضوعاً وصيغة وابدأ التوليد" : "Pick a topic and a modality to start"}
              </div>
