...
import {
  Card, Btn, Chip, inputStyle, textareaStyle, Toggle, StatusPill,
...
import { QuestionDef, evaluateAnswer, COURSE_BY_ID } from "../../data/instructorModule";
...
// identical component the review screen uses (spec 4.4).
// ─────────────────────────────────────────────────────────────────────────────
...
export default function AssignmentCreateScreen({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  const [status, setStatus] = useState<"draft" | "open" | "closed">(editing?.status ?? "open");
  ...
  const finish = (msg: string) => {
    toast(msg);
    setState({ ...state, screen: "course-workspace", tab: "assignments", assignmentId: undefined });
  };

  const save = (as: "draft" | "publish" | "changes") => {
    setTouched(true);
    if (!canSave) return;
    ...
    if (as === "changes" && editing) {
      ...
      finish(lang === "ar" ? "حُفظت تعديلات التكليف." : "Assignment changes saved.");
      return;
    }
    if (as === "draft") {
      publishAssignment(courseId, { titleEn: title.trim(), titleAr: title.trim(), showScore, questions: defs }, "draft");
      finish(lang === "ar" ? "حُفظ التكليف كمسودة — لن يراه الطلاب." : "Assignment saved as draft — students cannot see it.");
      return;
    }
    ...
    setState({ ...state, screen: "assignment-review", assignmentId: mod.assignments[0]?.id === undefined ? undefined : state.assignmentId, tab: "assignments" });
  };
  ...
  return (
    <div className="genai-pad" style={{ padding: "26px 32px", direction: isRtl ? "rtl" : "ltr" }}>
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start", marginBottom: 22, flexDirection: isRtl ? "row-reverse" : "row" }}>
        <BackCircle tokens={tokens} rtl={isRtl} onClick={() => setState({ ...state, screen: "course-workspace", tab: "assignments", assignmentId: undefined })} />
      ...
      <div className="genai-grid-builder" style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 440px", gap: 24, alignItems: "start" }}>
        {/* ── Authoring column ── */}
        <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <Card tokens={tokens} key={q.key} style={{ padding: "18px 20px" }}>
              ...
              <div style={{ marginTop: 16, padding: "14px 14px", background: tokens.inset, border: `1px dashed ${tokens.cardBorder}`, borderRadius: 10 }}>
                <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4, flexDirection: isRtl ? "row-reverse" : "row" }}>
                  ...
                  <span style={{ fontFamily: bFont, fontSize: 10.5, color: tokens.textMuted }}>· {lang === "ar" ? "اختياري" : "optional"}</span>
                </div>
          ...
          {accuracyEmpty && (
            <div style={{ fontFamily: bFont, fontSize: 11.5, color: tokens.textMuted, lineHeight: 1.6, padding: "10px 14px", background: tokens.inset, borderRadius: 10, border: `1px solid ${tokens.cardBorder}`, textAlign: isRtl ? "right" : "left" }}>
              {lang === "ar"
                ? "سيقول النظام عمله حتى بدون إجابة مرجعية أو روبرك، لكن بثقة أقل — وإن لم توجد مواد كافية للموضوع فستذهب الإجابات مباشرة إلى مراجعتك اليدوية."
          ...
          <div style={{ display: "flex", gap: 10, flexDirection: isRtl ? "row-reverse" : "row" }}>
            {editing ? (
              ...
              <>
                <Btn tokens={tokens} lang={lang} variant="ghost" onClick={() => save("draft")} style={{ padding: "12px 18px", fontSize: 13 }}>
