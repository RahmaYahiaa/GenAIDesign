...
import type { Lang } from "../data/instructorModule";
import { STUDENTS, studentTopicMastery } from "../data/instructorModule";
import { useInstructorModule } from "../store/InstructorStore";
import { Modal, Btn, textareaStyle, bFontFor, toast } from "./ModuleUI";
import { IconWarning, IconSparkle } from "./Icons";
import RemedialModal, { RemedialEntry } from "./RemedialModal";

// ─────────────────────────────────────────────────────────────────────────────
// Intervention modal — what the "Intervene" button actually opens: the
// student's snapshot for this course plus concrete next actions (generate
// remedial on the primary gap, queue an outreach note, open the full file).
// ─────────────────────────────────────────────────────────────────────────────

export default function StudentInterventionModal({ open, onClose, studentId, courseId, tokens, lang, onOpenFile }: {
  open: boolean; onClose: () => void; studentId: string | null; courseId: string;
  tokens: Tokens; lang: Lang; onOpenFile?: (id: string) => void;
}) {
  const { state: mod } = useInstructorModule();
  const bFont = bFontFor(lang);
  const isRtl = lang === "ar";
  const [note, setNote] = useState("");
  const [remedialFor, setRemedialFor] = useState<RemedialEntry | null>(null);
