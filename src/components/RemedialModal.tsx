...
// points (FR-CONTENT-01): from a detected misconception (pre-filled) or from
// analytics (topic gap). Output is always an editable draft that is NEVER
// visible to students automatically (FR-CONTENT-04); audience is chosen at
// publish time (FR-CONTENT-05).
// ─────────────────────────────────────────────────────────────────────────────
...
function draftBody(entry: RemedialEntry, type: "explanation" | "practice", lang: Lang): string {
...
function RadioRow({ on, title, sub, onClick, tokens, lang }: {
  on: boolean; title: string; sub?: string; onClick: () => void; tokens: Tokens; lang: Lang;
}) {
...
export default function RemedialModal({ open, onClose, entry, tokens, lang }: {
  open: boolean; onClose: () => void; entry: RemedialEntry | null; tokens: Tokens; lang: Lang;
}) {
  ...
  const [audience, setAudience] = useState<"all" | "affected" | "manual">("affected");
  const [manualIds, setManualIds] = useState<string[]>([]);

  // (re)initialise whenever the modal opens with an entry
  useEffect(() => {
  ...
  const generate = () => {
    ...
    setDraft(d);
    saveRemedial(d);                       // draft persists, invisible to students
  };

  const patchDraft = (patch: Partial<RemedialDraft>) => {
    if (!draft) return;
  ...
  const publish = () => {
    ...
    publishRemedial(draft.id);             // FR-CONTENT-05 — explicit publish
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 18, justifyContent: "flex-start", flexDirection: isRtl ? "row-reverse" : "row" }}>
